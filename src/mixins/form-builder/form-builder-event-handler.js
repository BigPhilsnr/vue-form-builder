...
const FORM_BUILDER_EVENT_HANDLER = {
  methods: {
    ...
    sectionPushedUp(sectionObj, type = 0) {
      ...
      // swap now
      this.$set(
        this.formData.sections[sectionObj.uniqueId],
        "sortOrder",
        preSectionOrder
      );
      this.$set(
        this.formData.sections[preSection.uniqueId],
        "sortOrder",
        postSortOrder
      );
    ...
    sectionUpdate(sectionObj) {
      ...
      this.formData.sections[sectionId] = Object.assign(
    ...
    controlNewAdded(sectionId, controlObj) {
      ...
      if (
        this.formData.sections[sectionId].controls.indexOf(
          controlObj.uniqueId
        ) === -1
      ) {
    ...
    controlDeletion(parentId, controlId) {
      ...
      if (type === "section") {
        ...
        let indexInSection = HELPER.findIndex(
          this.formData.sections[parentId].controls,
          undefined,
          controlId
        );
        ...
        let indexInRow = HELPER.findIndex(
          this.formData.rows[parentId].controls,
          undefined,
          controlId
        );
      ...
      this.$formEvent.$emit(
        EVENT_CONSTANTS.BUILDER.CONTROL.DELETED,
        parentId,
        controlId
      );
    },
    ...
    controlUpdated(controlId, controlData) {
      ...
      // update by using the extend . best way
      this.formData.controls[controlId] = Object.assign(
        this.formData.controls[controlId],
        controlData
      );
    },
  },
  ...
  created() {
    ...
    this.$formEvent.$on(
      ...
      this.sectionAndRowMapping
    );
    this.$formEvent.$on(
      EVENT_CONSTANTS.BUILDER.SECTION.PUSH,
      this.sectionPushedUp
    );
    this.$formEvent.$on(
    ...
    this.$formEvent.$on(
      EVENT_CONSTANTS.BUILDER.SECTION.UPDATE,
      this.sectionUpdate
    );
...
export { FORM_BUILDER_EVENT_HANDLER };
